...
var Sandbox = {
  ...
  Model: Backbone.Model.extend({
    ...
    iframeSetup: function() {
      // Check for skillcrush iframe else create dynamic one
      if ($('#sclmsaddon-editor-preview-iframe').length > 0) {
        this.sandboxFrame = $('#sclmsaddon-editor-preview-iframe')[0];
      } else {
      ...
      // send console statements to sandbox
      this.sandbox.console.log = function() {
        var args = Array.prototype.slice.call(arguments);
        args.map(function(arg) {
          sandbox.model.displayMessage(arg.toString(), 'string');
        });
        return;
      };
      this.sandbox.console.error = function() {
        var args = Array.prototype.slice.call(arguments);
        args.map(function(arg) {
          sandbox.model.displayMessage(arg.toString(), 'error');
        });
        return;
      };
      this.sandbox.console.warn = function() {
        var args = Array.prototype.slice.call(arguments);
        args.map(function(arg) {
          sandbox.model.displayMessage('Warning: ' + arg.toString(), 'warn');
        });
        return;
      };
      this.sandbox.console.info = function() {
        var args = Array.prototype.slice.call(arguments);
        args.map(function(arg) {
          sandbox.model.displayMessage('Info: ' + arg.toString(), 'info');
        });
        return;
      };

      // prevent console messages from appearing in browser console
      window.console.log = window.console.error = window.console.warn = window.console.info = window.onerror = function() {
        return true;
      };
    ...
    iframeEval: function(command) {
      ...
      return this.sandbox.eval
        ...
        : this.get('fallback')
          ? eval(command)
          : new Error("Can't evaluate inside the iframe - please report this bug along with your browser information!");
    },
    ...
    evaluate: function(command) {
      ...
      try {
        ...
        $('.sclmsaddon-console-tab a').css('background-color', '#d36250');
        $('.sclmsaddon-console-tab-updated a').attr('has-js-error', 'true');
    ...
    // 	Display messages in the sandbox
    //	We receive the text and the type of message (class)
    displayMessage: function(text, _class) {
      var item = {
        command: text,
      };
